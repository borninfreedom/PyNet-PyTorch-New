...
class LoadData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pil_image = Image.fromarray(dslr_image)
        new_size = (int(pil_image.width * (self.scale / 2.0)), int(pil_image.height * (self.scale / 2.0)))
        resized_image = pil_image.resize(new_size, Image.BICUBIC)
        dslr_image = np.array(resized_image)
        # dslr_image = np.float32(np.array(Image.fromarray(dslr_image).resize()/ 255.0
